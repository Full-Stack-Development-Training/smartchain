...
const EXECUTE_COMPLETE = "Execution complete";
const EXECUTE_LIMIT = 10000;
...
class Interpreter {
  ...
  runCode(code) {
    ...
    while (this.state.programCounter < this.state.code.length) {
      ...
      if (this.state.executionCount > EXECUTE_LIMIT) {
        throw new Error(
          `Check for infinite loop. Execution limit of ${EXECUTE_LIMIT} has been exceeded.`
        );
      }
      ...
      try {
        switch (opCode) {
          case STOP:
            throw new Error(EXECUTE_COMPLETE);
          case PUSH:
            this.state.programCounter++;
        ...
        if (error.message === EXECUTE_COMPLETE) {
